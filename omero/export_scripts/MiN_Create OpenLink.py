...
OPENLINK_DIR = "/opt/omero/omero_openlink_dir"

# name of nginx website
SERVER_NAME = "omero-imaging.uni-muenster.de"
...
# email originator
ADMIN_EMAIL = "thomas.zobel@uni-muenster.de"
...
def get_omero_paths(client):
    ...
    managed_repo_dir = '/opt/omero/data_dir/ManagedRepository';
    orig_repo_dir = '/opt/omero/data_dir/';
    ...
    if not managed_repo_dir:
        print(
            "ERROR: no specification was found for managed repository path. Please check path of type Managed under \n >>omero fs repos \n or the value of omero.managed.dir under\n >>omero config get")
        return None, None

    if not orig_repo_dir:
        print(
            "ERROR: no specification was found for omero repository path. Please check path of type Public under \n >>omero fs repos \n or the value of omero.data.dir under\n >>omero config get")
        return None, None
        ...
        for file in fileList:
            ...
            if not os.path.basename(file) == os.path.basename(curlFile):
                ...
                if len(relPath) > MAX_PATHLENGTH:
                    print(
                        "ATTENTION: pathlength is in the critical range! This could generate download issues for %s" % relPath)
...
def createObjectDir(type, ppath, object, name):
    ...
    if not os.path.exists(path):
        try:
            ...
            print(
                "ERROR: Cannot create directory for %s: %s in %s (possible problems:length of name, usage of /\:*?''<>|)" % (
                type, name, ppath))
            return None
    else:
